...
import { Zap, Sparkles, AlertCircle, Copy, Check, Globe } from 'lucide-react';
...
export const AuthScreen: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-zello-black text-slate-100 flex flex-col items-center justify-center p-8 relative overflow-hidden font-sans border-8 border-zello-black/20">
      ...
      <div className="absolute top-6 right-6 z-20 flex items-center bg-white/5 border border-white/10 rounded-xl p-1 gap-1">
      ...
      >
        <div className="space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zello-orange/10 border border-zello-orange/20 text-zello-orange text-xs font-black uppercase tracking-widest">
            ...
            {t('auth.freeAccess', { defaultValue: 'Acesso Livre' })}
          </div>
          ...
          <p className="text-slate-400 font-medium">
            {t('auth.syncProfile', { defaultValue: 'Sincronize seu perfil para salvar seu progresso, dominar Estruturas Libertadoras e subir no ranking.' })}
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full py-5 bg-zello-orange text-white font-black uppercase tracking-[0.2em] text-sm rounded-3xl hover:brightness-110 transition-all active:scale-95 shadow-[0_0_40px_rgba(240,90,40,0.3)] flex items-center justify-center gap-3 cursor-pointer"
          >
            ...
            {t('auth.signInGoogle', { defaultValue: 'Entrar com Google' })}
          </button>
          
          <div className="flex items-center gap-4 py-4">
            <div className="h-px flex-1 bg-white/10"></div>
            <span className="text-[10px] font-black uppercase text-slate-600 tracking-widest">{t('auth.or', { defaultValue: 'ou' })}</span>
            <div className="h-px flex-1 bg-white/10"></div>
          </div>

          <button
            onClick={handleGuestLogin}
            className="w-full py-4 bg-white/5 border-2 border-white/10 text-slate-400 font-black uppercase tracking-[0.2em] text-xs rounded-3xl hover:border-white/30 hover:text-white transition-all active:scale-95 cursor-pointer"
          >
            {t('auth.guestMode', { defaultValue: 'Modo Visitante (Sem Salvar Nuvem)' })}
          </button>
        ...
        <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest leading-relaxed">
          {t('auth.terms', { defaultValue: 'Ao entrar, você concorda com os termos de letramento digital e conformidade ética de IA.' })}
        </p>
      </motion.div>
